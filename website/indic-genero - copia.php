...
?>
<?php
// ==== Contador de visitas para indic-genero.php ====
// Cuenta 1 visita por usuario cada $throttleSeconds (12h por defecto).
function incrementar_contador_pagina(string $clave, int $throttleSeconds = 43200): int {
    $baseDir = __DIR__ . '/storage/counters';
    if (!is_dir($baseDir)) {
        @mkdir($baseDir, 0755, true);
    }
    $archivo = $baseDir . "/{$clave}.count";
    $cookieName = "vc_" . preg_replace('/[^a-z0-9_\-]/i', '_', $clave);

    // Si no existe cookie reciente, incrementa
    if (empty($_COOKIE[$cookieName])) {
        $fp = @fopen($archivo, 'c+'); // crea si no existe
        if ($fp) {
            if (flock($fp, LOCK_EX)) {
                // Lee valor actual
                $contenido = '';
            ...
            // Si no puede abrir archivo, no romper la página: retorna 0
        }

        // Coloca cookie de “enfriamiento”
        setcookie($cookieName, '1', [
            'expires'  => time() + $throttleSeconds,
            'path'     => '/',
            'samesite' => 'Lax',
        ]);
    }

    // Lee el total para mostrarlo
    $total = 0;
    if (is_file($archivo)) {
        $total = (int)trim(@file_get_contents($archivo));
    }
    return $total;
}

// Úsalo con una clave única para este PHP
$visitasIndicGenero = incrementar_contador_pagina('indic-genero', 43200); // 12h
?>
...
<div id="main-body" class="main-body">
  ...
  <span class="icon">🧭</span> Personas visitando nuestro Observatorio:
  <span class="badge bg-secondary" style="margin-left:.5rem;">
    <?= number_format($visitasIndicGenero) ?> visitas
  </span>
</div>
                ...
                <div class="col-12 col-sm-12">
                    <h2 class="indicador-title">
                        Dimensión Asuntos de Genero
                    </h2>
                    <div class="indicador-description">
                        <p class="description">
                            En esta dimensión se reúnen y analizan datos con enfoque de género, reconociendo la importancia de visibilizar las desigualdades y brechas que afectan de manera diferenciada a mujeres y hombres. Adoptar la perspectiva de género permite comprender cómo los roles y estereotipos asignados influyen en el acceso a derechos, recursos y oportunidades, y aporta información clave para el diseño de políticas públicas más equitativas e incluyentes.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <!-- Sistema de Pestañas con JavaScript y CSS -->
            <div class="container-fluid mt-4">
                <div class="tabs">
                    <button class="tab-link active" onclick="openTab(event, 'marco')">Marco institucional</button>
                    <button class="tab-link" onclick="openTab(event, 'informacion')">Información</button>
                    <button class="tab-link" onclick="openTab(event, 'seguimiento')">Seguimiento</button>
                    <button class="tab-link" onclick="openTab(event, 'ruta')">Ruta de atención</button>
                    <button class="tab-link" onclick="openTab(event, 'barreras')">Barreras de acceso</button>
                    <button class="tab-link" onclick="openTab(event, 'atencion')">Atención integral</button>
                    <button class="tab-link" onclick="openTab(event, 'politica')">Política Pública</button>
                    <button class="tab-link" onclick="openTab(event, 'campana')">Campañas y publicaciones</button>
                    <button class="tab-link" onclick="openTab(event, 'reportes')">Reportes sociales</button>
                </div>

                <div class="tab-content">
                    <!-- Pestaña Buscador y Categorías -->
                    <div id="marco" class="tab-pane active">
                        <div style="display: flex; align-items: center;">
                            <img src="assets/svg/img-genero/MARCO-INSTITUCIONAL.svg" alt="Icono Marco Institucional" style="width: 50px; height: 50px; margin-right: 15px;">
                            <h2 style="margin: 0;" class="indicador-title">Marco Institucional</h2>
                        </div>
            ...
            <!-- Pestaña Ruta de atención -->
                <div id="ruta" class="tab-pane">
                <div class="indicadores-section mt-4">
                    <div style="display: flex; align-items: center;">
    ...
    <div class="info-grid">
      ...
      <div class="info-value">
        Protección del derecho a la <em>verdad, justicia y reparación integral</em> para todas las víctimas.
            ...
            <!-- Pestaña barreras de acceso -->
            <div id="barreras" class="tab-pane">
                <div class="indicadores-section mt-4">
                <div style="display: flex; align-items: center;">
            ...
            <div id="atencion" class="tab-pane">
                ...
                <div id="politica" class="tab-pane">
                    <div class="indicadores-section mt-4">
                        <div style="display: flex; align-items: center;">
            ...
            <!-- Inicio Pestaña campañas y publicaciones -->
            <div id="campana" class="tab-pane">
                <div class="indicadores-section mt-4">
                    <div style="display: flex; align-items: center;">
